...
/** V1.2 legacy path: priority columns via ensureColumn before user_version was introduced. */
...
async function migrateV5CleanupLegacySchema(db: Database): Promise<void> {
  ...
  const hasLegacyChecklistColumns = checklistColumns.some(
    (column) => column.name === 'mode' || column.name === 'scheduled_for',
  );
  ...
  const hasLegacyItemColumns = itemColumns.some(
    (column) => column.name === 'price' || column.name === 'quantity' || column.name === 'color',
  );
...
/** Sets `created_at` on legacy rows from parent checklist `created_at`. */
